...
class SpERTTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: Type[BaseInputReader]):
        ...
        compute_loss = SpERTLoss( \
                            ...
                            include_word_piece_task = args.include_word_piece_task
                            )
    ...
    def _load_model(self, input_reader):
        ...
        model = model_class.from_pretrained(self._args.model_path,
                                            ...
                                            sent_label_types = input_reader.sent_type_count,
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            batch_loss = compute_loss.compute( \
                                            ...
                                            # word_piece_logits = word_piece_logits,
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader, total=total, desc='Evaluate epoch %s' % epoch):
                ...
                result = model( \
                                ...
                                entity_sample_masks = batch['entity_sample_masks'],
                                inference = True)
                #entity_clf, subtype_clf, rel_clf, rels, sent_clf, word_piece_clf = result
                entity_clf, subtype_clf, rel_clf, rels, sent_clf = result
                #entity_clf  = result[0].detach().cpu().numpy()
                #subtype_clf = { k: v.detach().cpu().numpy() for k, v in result[1].items() }
                #rel_clf     = result[2].detach().cpu().numpy()
                #rels        = result[3].detach().cpu().numpy()
                #sent_clf    = result[4].detach().cpu().numpy()
                #batch       = util.to_device(batch, 'cpu')
                ...
                evaluator.eval_batch( \
                                    ...
                                    #batch_word_piece_clf = word_piece_clf,
                                    batch = batch)
                del result, entity_clf, subtype_clf, rel_clf, rels, sent_clf, batch
                #reporter.report()
                #gc.collect()
